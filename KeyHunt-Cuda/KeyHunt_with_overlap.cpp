...
void KeyHunt::Search(int nbThread, std::vector<int> gpuId, std::vector<int> gridSize, bool& should_exit)
{
	...
	Int overlap;
	overlap.SetInt64(50000); // Add 50,000 keys overlap
	rangeStart.Add(&rangeDiff);
	Int tempRangeEnd = rangeStart;
	tempRangeEnd.Add(&overlap); // Apply overlap to rangeEnd
	params[i].rangeEnd.Set(&tempRangeEnd);
	...
	Int overlap;
	overlap.SetInt64(5000000); // Add 5,000,000 keys overlap
	rangeStart.Add(&rangeDiff);
	Int tempRangeEnd = rangeStart;
	tempRangeEnd.Add(&overlap); // Apply overlap to rangeEnd
	params[i].rangeEnd.Set(&tempRangeEnd);
